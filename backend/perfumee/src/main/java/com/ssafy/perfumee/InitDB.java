    ...
    for (int r = 0; r < rows; r++) {
      ...
      if (row != null) { // 행이 비어있지 않을 때
        ...
        for (int c = 0; c < cells; c++) {
          ...
          if (c == 1) {
            ...
            if (image == null || image.length() == 0) {
              ...
              image = image.substring("https://basenotes.com/img/product/".length());
            }
        ...
        for (String s : strM) {
          if(s.isEmpty()) continue;
          Optional<Note> note = noteRepository.findByName(s.trim().toLowerCase().replace("\"", ""));
          Contain contain = new Contain(perfume, "middle", note.get());
          containRepository.save(contain);
        }
        ...
        for (String s : strB) {
          if(s.isEmpty()) continue;
          Optional<Note> note = noteRepository.findByName(s.trim().toLowerCase().replace("\"", ""));
          Contain contain = new Contain(perfume, "base", note.get());
          containRepository.save(contain);
        }
